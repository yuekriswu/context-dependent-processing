...
def setup():
    ...
    model = nn.V1(
        ...
        f=nn.Match(
            cases={
                "PYR": nn.Compose(nn.Pow(1, optim=False, bounds=[1.0, 5.0], tag="p_PYR"), nn.Rectified()),
                "PV": nn.Compose(nn.Pow(1, optim=False, bounds=[1.0, 5.0], tag="p_PV"), nn.Rectified()),
                "SST": nn.Compose(nn.Pow(1, optim=False, bounds=[1.0, 5.0], tag="p_SST"), nn.Rectified()),
                "VIP": nn.Compose(nn.Pow(1, optim=False, bounds=[1.0, 5.0], tag="p_VIP"), nn.Rectified()),
...
def get_inputs(x):
    ...
    for stim_type_idx in l_stim_type:
        ...
        for stim_idx in np.arange(9):
            ...
            H_X, H_X2 = define_X_Y_inputs(para_X, stim_idx, dist_mat)

            H_X_reshaped = H_X.reshape(30, 30)
            H_X2_reshaped = H_X2.reshape(30, 30)

            if config == 'classical_only' or config == 'inverse_only':
                inputs[stim_idx, 4, :, :] = H_L4  # inputs from L4
                inputs[stim_idx, 5, :, :] = H_LM  # inputs from LM
                inputs[stim_idx, 6, :, :] = H_X_reshaped  # inputs from X
                inputs[stim_idx, 7, :, :] = H_X2_reshaped  # inputs from Y
            else:
                inputs[stim_type_idx * 9 + stim_idx, 4, :, :] = H_L4  # inputs from L4
                inputs[stim_type_idx * 9 + stim_idx, 5, :, :] = H_LM  # inputs from LM
                inputs[stim_type_idx * 9 + stim_idx, 6, :, :] = H_X_reshaped  # inputs from X
                inputs[stim_type_idx * 9 + stim_idx, 7, :, :] = H_X2_reshaped  # inputs from Y
...
def main():
    ...
    losses, state_dicts = fit.run(
        ...
        N=10,
        progress=True,
        out="../optimized_files/optimized_files_final_gaussian_classical_linear_2",
        loss_threshold=0.6,
