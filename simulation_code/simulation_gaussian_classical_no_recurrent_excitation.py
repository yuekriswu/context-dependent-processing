...
l_fitting_config = ['gaussian_classical_no_recurrent_excitation']
l_factor = [1/8]
...
def read_rf_parameters_X(stim_type_idx, factor):
    para_X = torch.zeros(4, 9)
    if stim_type_idx == 0:
        para_X[0, :] = factor * torch.sqrt(torch.tensor([5, 15, 25, 35, 45, 55, 65, 75, 85]))
        para_X[2, :] = torch.full((9,), 15.0)
        para_X[3, :] = torch.full((9,), 15.0)

        # para_X[0, :] = torch.sqrt(torch.tensor([5, 15, 25, 35, 45, 55, 65, 75, 85]))
...
def setup(model_name, fitting_config):
    ...
    cell_types = [CellType.PYR, CellType.PV, CellType.SST, CellType.VIP, CellType.L4, CellType.LM, CellType.X, CellType.Y]
        ...
        "gW": torch.tensor(
            [
                [2 * np.sqrt(0.5996) * np.abs(opti_networks[0, 0]),
                 ...
                 2 * np.sqrt(0.5996) * np.abs(opti_networks[0, 6]),
                 2 * np.sqrt(0.5996) * np.abs(opti_networks[0, 7])],
                [2 * np.sqrt(1.35) * np.abs(opti_networks[1, 0]),
                 ...
                 2 * np.sqrt(1.35) * np.abs(opti_networks[1, 6]),
                 2 * np.sqrt(1.35) * np.abs(opti_networks[1, 7])],
                [2 * np.sqrt(0.6818) * np.abs(opti_networks[2, 0]),
                 ...
                 2 * np.sqrt(0.6818) * np.abs(opti_networks[2, 6]),
                 2 * np.sqrt(0.6818) * np.abs(opti_networks[2, 7])],
                [2 * np.sqrt(0.8437) * np.abs(opti_networks[3, 0]),
                 ...
                 2 * np.sqrt(0.8437) * np.abs(opti_networks[3, 6]),
                 2 * np.sqrt(0.8437) * np.abs(opti_networks[3, 7])],
...
def get_inputs(x, factor):
    ...
    for stim_type_idx in l_stim_type:
        ...
        para_X = read_rf_parameters_X(stim_type_idx, factor)
        ...
        for stim_idx in range(9):
            ...
            inputs[stim_idx, -4, :, :] = H_L4_reshaped  # inputs from L4
            inputs[stim_idx, -3, :, :] = H_LM_reshaped  # inputs from LM
...
def main():
    for fitting_config_idx in range(len(l_fitting_config)):
        ...
        factor = l_factor[fitting_config_idx]
